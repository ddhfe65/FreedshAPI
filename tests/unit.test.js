...
test('splitPromptIntoChunks partitions oversized prompt along safe boundaries', () => {
  const shortText = 'Hello DeepSeek! This is short.';
  assert.deepEqual(serverInternals.splitPromptIntoChunks(shortText, 100), [shortText]);

  const line1 = 'A'.repeat(60000);
  const line2 = 'B'.repeat(60000);
  const line3 = 'C'.repeat(30000);
  const joined = `${line1}\n${line2}\n${line3}`;

  const chunks = serverInternals.splitPromptIntoChunks(joined, 72000);
  assert.equal(chunks.length, 3);
  assert.equal(chunks[0], line1);
  assert.equal(chunks[1], line2);
  assert.equal(chunks[2], line3);
});

test('pipeDeepSeekStreamToClient streams OpenAI text and reasoning deltas directly', async () => {
  const chunksWritten = [];
  const res = {
    headersSent: false,
    writeHead: () => { res.headersSent = true; },
    write: (chunk) => { chunksWritten.push(chunk); },
    end: () => { res.writableEnded = true; },
    on: () => {},
  };

  // Mock upstream DeepSeek SSE stream with reasoning and content
  const mockReadable = (async function* () {
    yield Buffer.from('data: {"v":{"response":{"message_id":"msg_123"}}}\n\n');
    yield Buffer.from('data: {"p":"response/fragments","v":[{"type":"THINK","content":"thinking"}]}\n\n');
    yield Buffer.from('data: {"p":"response/fragments/-1/content","v":" more"}\n\n');
    yield Buffer.from('data: {"p":"response/fragments","v":[{"type":"RESPONSE","content":"Hello"}]}\n\n');
    yield Buffer.from('data: {"p":"response/fragments/-1/content","v":" world"}\n\n');
    yield Buffer.from('data: {"p":"response/status","v":"FINISHED"}\n\n');
  })();

  const session = serverInternals.createSession();
  await serverInternals.pipeDeepSeekStreamToClient({ body: mockReadable }, res, {
    apiMode: 'openai',
  ...
  assert.equal(session.parentMessageId, 'msg_123');
  assert.equal(session.messageCount, 1);
  assert.ok(res.writableEnded);

  const combined = chunksWritten.join('');
  assert.match(combined, /reasoning_content":" more"/);
  assert.match(combined, /content":" world"/);
  assert.match(combined, /finish_reason":"stop"/);
  assert.match(combined, /\[DONE\]/);
});

test('parseToolCall accepts singular tool_call wrapper with DSML and Windows paths', () => {
  const dsml = [
    '<tool_call>',
    '<invoke name="read">',
    '<parameter name="file_path">C:\\Users\\dimak\\Desktop\\FreedshAPI\\server.js</parameter>',
    '</invoke>',
    '</tool_call>',
  ].join('\n');
  ...
  assert.equal(args.file_path, 'C:\\Users\\dimak\\Desktop\\FreedshAPI\\server.js');
});

test('parseToolCall accepts singular tool_call wrapper with inner JSON', () => {
  const jsonMarkup = [
    '<tool_call>',
    '{"name": "write", "arguments": {"path": "C:\\\\tmp\\\\file.txt", "content": "hello"}}',
    '</tool_call>',
  ].join('\n');

  const call = serverInternals.parseToolCall(jsonMarkup);
  assert.ok(call);
  assert.equal(call.name, 'write');
  const args = JSON.parse(call.arguments);
  assert.equal(args.path, 'C:\\tmp\\file.txt');
  assert.equal(args.content, 'hello');
});

test('extractSessionPrompt returns full prompt on turn 1 and delta on subsequent turns', () => {
  const tools = [{
    type: 'function',
    function: { name: 'read', description: 'Read a file', parameters: { type: 'object', properties: { file_path: { type: 'string' } } } }
  }];

  const turn1Messages = [
    { role: 'system', content: 'You are an assistant.' },
    { role: 'user', content: 'Read server.js please' }
  ];

  const session = serverInternals.createSession();

  // Turn 1: New session without id
  const turn1 = serverInternals.extractSessionPrompt(turn1Messages, tools, session);
  assert.equal(turn1.isDelta, false);
  assert.match(turn1.prompt, /--- TOOL REQUEST SYSTEM ---/);
  assert.match(turn1.prompt, /Read server\.js please/);

  // Simulate DeepSeek creating the session and answering
  session.id = 'session-123';
  session.parentMessageId = 'msg-1';
  session.messageCount = 1;

  // Turn 2: Client sends assistant tool call + tool result
  const turn2Messages = [
    { role: 'system', content: 'You are an assistant.' },
    { role: 'user', content: 'Read server.js please' },
    { role: 'assistant', content: null, tool_calls: [{ id: 'call_1', function: { name: 'read', arguments: '{"file_path":"server.js"}' } }] },
    { role: 'tool', content: 'const server = require("http"); // file content' }
  ];

  const turn2 = serverInternals.extractSessionPrompt(turn2Messages, tools, session);
  assert.equal(turn2.isDelta, true);
  // Must contain only the tool result
  assert.equal(turn2.prompt, '[Tool Result]\nconst server = require("http"); // file content');
  // Must NOT contain repeated system prompt or old user message
  assert.doesNotMatch(turn2.prompt, /--- TOOL REQUEST SYSTEM ---/);
  assert.doesNotMatch(turn2.prompt, /Read server\.js please/);
  // Fresh prompt must still be available as fallback
  assert.match(turn2.freshPrompt, /--- TOOL REQUEST SYSTEM ---/);
  assert.match(turn2.freshPrompt, /Read server\.js please/);

  // Turn 3: Client starts a brand new conversation without assistant turns
  const turn3NewConvo = [
    { role: 'system', content: 'You are an assistant.' },
    { role: 'user', content: 'New chat: tell me a joke' }
  ];

  const turn3 = serverInternals.extractSessionPrompt(turn3NewConvo, tools, session);
  // Old remote session should be reset
  assert.equal(session.id, null);
  assert.equal(turn3.isDelta, false);
  assert.match(turn3.prompt, /New chat: tell me a joke/);
});

test('parseToolCall auto-coerces numbers, booleans, and nulls without string="false"', () => {
  const dsml = [
    '<tool_call>',
    '<invoke name="read">',
    '<parameter name="file_path">C:\\Users\\dimak\\Desktop\\FreedshAPI\\server.js</parameter>',
    '<parameter name="offset">1145</parameter>',
    '<parameter name="limit">1200</parameter>',
  ...
  const args = JSON.parse(call.arguments);
  assert.equal(args.file_path, 'C:\\Users\\dimak\\Desktop\\FreedshAPI\\server.js');
  assert.strictEqual(args.offset, 1145);
  assert.strictEqual(args.limit, 1200);
  assert.strictEqual(args.enabled, true);
  assert.strictEqual(args.optional, null);
});

test('pipeDeepSeekStreamToClient preserves initial fragment content without clipping starting characters', async () => {
  const chunksWritten = [];
  const res = {
    headersSent: false,
    writeHead: () => { res.headersSent = true; },
  ...
  // Upstream DeepSeek SSE stream sends initial character in response/fragments, then appends to it
  const mockReadable = (async function* () {
    yield Buffer.from('data: {"v":{"response":{"message_id":"msg_456"}}}\n\n');
    yield Buffer.from('data: {"p":"response/fragments","v":[{"type":"THINK","content":"мы"}]}\n\n');
    yield Buffer.from('data: {"p":"response/fragments/-1/content","v":" должны протестировать"}\n\n');
    yield Buffer.from('data: {"p":"response/fragments","v":[{"type":"RESPONSE","content":"К"}]}\n\n');
    yield Buffer.from('data: {"p":"response/fragments/-1/content","v":"ратко: всё работает безупречно"}\n\n');
    yield Buffer.from('data: {"p":"response/status","v":"FINISHED"}\n\n');
  })();
  ...
  const combined = chunksWritten.join('');
  // Assert reasoning starts with 'мы'
  assert.match(combined, /reasoning_content":"мы"/);
  // Assert text contains 'К' and 'ратко' so full text is 'Кратко'
  assert.match(combined, /content":"К"/);
  assert.match(combined, /content":"ратко/);
});

test('pipeDeepSeekStreamToClient resets remote session on empty response (0 chars)', async () => {
  const res = {
    headersSent: false,
    writeHead: () => { res.headersSent = true; },
    write: () => {},
  ...
  // Upstream returns 0 tokens
  const mockEmpty = (async function* () {
    yield Buffer.from('data: {"p":"response/status","v":"FINISHED"}\n\n');
  })();

  const session = serverInternals.createSession();
  session.id = 'stuck-session-123';
  session.parentMessageId = '38';

  await serverInternals.pipeDeepSeekStreamToClient({ body: mockEmpty }, res, {
    apiMode: 'openai',
    requestedModel: 'deepseek-chat',
    agentId: 'test-agent',
    session,
  ...
  // Session must be reset so retries do not loop on stuck parent: 38
  assert.equal(session.id, null);
  assert.equal(session.parentMessageId, null);
});

test('extractSessionPrompt auto-resets remote session when compaction checkpoint is detected', () => {
  const tools = [];
  const session = serverInternals.createSession();
  session.id = 'old-bloated-chat';
  session.parentMessageId = 'msg-25';

  const compactedMessages = [
    { role: 'system', content: 'You are an assistant.' },
    { role: 'user', content: 'This is an automatically generated checkpoint condensing an earlier span of the conversation.\n\n<compacted-summary>\n## Primary Request and Intent\n- Fix bugs\n</compacted-summary>' },
    { role: 'user', content: 'Continue from checkpoint' }
  ];

  const result = serverInternals.extractSessionPrompt(compactedMessages, tools, session);
  // Remote session must be reset to force creating a clean chat in DeepSeek Web
  assert.equal(session.id, null);
  assert.equal(result.isDelta, false);
  assert.match(result.prompt, /<compacted-summary>/);
});

test('parseToolCall accepts unclosed tool_calls wrapper with invoke (screenshots #3 & #4)', () => {
  const readSample = '<tool_calls> <invoke name="read"> <parameter name="file_path">C:\\Users\\dimak\\Desktop\\FreedshAPI\\client.js</parameter> </invoke>';
  const readTc = serverInternals.parseToolCall(readSample);
  assert.ok(readTc, 'Expected read tool call to be parsed despite missing </tool_calls>');
  assert.equal(readTc.name, 'read');
  const readArgs = JSON.parse(readTc.arguments);
  assert.equal(readArgs.file_path, 'C:\\Users\\dimak\\Desktop\\FreedshAPI\\client.js');

  const pwshSample = '<tool_calls> <invoke name="pwsh"> <parameter name="command">p = \'C:\\Users\\dimak\\Desktop\\FreedshAPI\\package.json\'; (Get-Item -LiteralPath $p).Length</parameter> <parameter name="description">Read package.json</parameter> </invoke>';
  const pwshTc = serverInternals.parseToolCall(pwshSample);
  assert.ok(pwshTc, 'Expected pwsh tool call to be parsed despite missing </tool_calls>');
  assert.equal(pwshTc.name, 'pwsh');
  const pwshArgs = JSON.parse(pwshTc.arguments);
  assert.match(pwshArgs.command, /package\.json/);
  assert.equal(pwshArgs.description, 'Read package.json');
});

test('parseToolCall accepts unclosed tool_calls wrapper with inner JSON', () => {
  const sample = '<tool_calls> {"name": "read", "arguments": {"file_path": "C:\\\\Users\\\\dimak\\\\Desktop\\\\FreedshAPI\\\\client.js"}}';
  const tc = serverInternals.parseToolCall(sample);
  assert.ok(tc, 'Expected JSON tool call to be parsed despite missing </tool_calls>');
  assert.equal(tc.name, 'read');
  const args = JSON.parse(tc.arguments);
  assert.equal(args.file_path, 'C:\\Users\\dimak\\Desktop\\FreedshAPI\\client.js');
});

test('extractSessionPrompt ignores compaction worker summarization prompt and preserves active session', () => {
  const tools = [];
  const session = serverInternals.createSession();
  session.id = 'active-chat-123';
  session.parentMessageId = 'msg-10';

  const workerMessages = [
    { role: 'user', content: 'Привет' },
    { role: 'assistant', content: 'Привет! Чем помочь?' },
    { role: 'user', content: 'You are now acting as a compaction engine for this AI coding assistant. Condense the conversation ABOVE into a structured checkpoint that lets another model resume the work with no loss of essential context.\n\nRules:\n- If the conversation already contains a <compacted-summary> block, it is a PRIOR checkpoint.' }
  ];

  const result = serverInternals.extractSessionPrompt(workerMessages, tools, session);
  // Must NOT reset session because this is the summarization worker prompt, not the resumed checkpoint
  assert.equal(session.id, 'active-chat-123');
  assert.equal(session.activeCheckpoint, null);
  assert.equal(result.isDelta, true);
  assert.match(result.prompt, /compaction engine/);
});
